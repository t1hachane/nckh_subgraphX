...
class MCTSNode():
    ...
    def __init__(self, coalition: list, data: Data,
        ...
        self.N = N  # times of arrival (C(N_i, a_j))
        self.P = P  # property score (reward (R(N_i, a_j)))
...
class MCTS():
    def __init__(self, X: torch.Tensor, edge_index: torch.Tensor, n_rollout: int,
        ...
        self.min_atoms = min_atoms # the minimum number of atoms in the sub-graph
        self.c_puct = c_puct
        self.expand_atoms = expand_atoms # the number of atoms to expand

        self.root_coalition = sorted([i for i in range(self.num_nodes)]) # tập coalition ban đầu

        # partial function to create a MCTSNode
        ...
        # state_map is a dictionary that maps the coalition to the node
